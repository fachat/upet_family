...
image::images/whitecase.jpg[The Ulti-PET]

There are two more smaller systems based on the same technology:
...
* In addition, the *Ultra-Bus* serves as an expansion bus for the Ulti-PET and as base board for the Ultra-CPU. It allows using certain Apple-II, RC2014, and C64 expansions cards aside from its native *CS/A* bus cards.

== Features

All variants center around a set of core features, as implemented in the Ultra-CPU board and the firmware. The core specs are:

=== Firmware

* all five main models supported: BASIC1, BASIC2, and BASIC4 40 columns, BASIC4 80 columns, and 8296.
* BASIC4 with some basic BASIC extensions
* Editor ROMs support Colour-PET modes (based on Steve Gray's editor ROM project)
* SD-Card support as own drive unit
* USB mouse or keyboard input
* configuration options stored in RTC Flash memory
* Run over serial line instead of attached keyboard/screen
...
This project is an open source / open hardware project. I don't sell any pre-made products, but provide all the information needed to
order (almost fully) populated boards from a PCB manufacturing house, and order the rest from electronics parts supplier.
The software is also included in the subdirectories here.

To get the information, you need to clone all all the subdirectories which are in their own repository (see below). 
Then you can see the different system folders for the PCBs (ultipet, ultracpu, ultrabus, micropet), as well FPGA configuration 
and firmware. Note that the subdirectories will also contain other subdirectories.

To actually build one, you need to

* get a populated PCB, e.g. from a PCB manufacturer. For production files see the system-specific directories.
* get the missing parts from an electronics supplier (details see the system-specific directories)
* install the parts - soldering required!
* program the FPGA firmware into an own Flash ROM to put on the PCB. It is an SPI Flash ROM that any modern programmer should support.
* build and program the system firmware (in the roms directory) into an own SPI Flash ROM
* attach a C64 keyboard, or a PET keyboard, or build a keyboard replacement (based on Steve's designs, but greatly enhanced, see below)
* either attach a fitting SD-Card reader and SD card, or an IEEE-based floppy drive or drive-emulator as mass storage.
* attach power, VGA monitor, switch on, and enjoy!

=== How to get the info

To build the boards, a few core git repositories are required that contain shared code (FPGA, Firmware) in addition to the
...
in the directory for this readme. This will also recursively include all sub-sub-directories required.
